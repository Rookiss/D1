#include "D1GameplayAbility_ThrowTorch.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(D1GameplayAbility_ThrowTorch)

UD1GameplayAbility_ThrowTorch::UD1GameplayAbility_ThrowTorch(const FObjectInitializer& ObjectInitializer)
...
void UD1GameplayAbility_ThrowTorch::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
	ClearCameraMode();
...
void UD1GameplayAbility_ThrowTorch::OnInputRelease(float TimeHeld)
{
...
void UD1GameplayAbility_ThrowTorch::OnInputConfirm()
{
	if (InputReleaseTask)
	{
...
void UD1GameplayAbility_ThrowTorch::OnThrowFinished()
{
...
void UD1GameplayAbility_ThrowTorch::OnThrowEndFinished()
{
	CancelAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true);
}

void UD1GameplayAbility_ThrowTorch::OnThrowEvent(FGameplayEventData Payload)
{
	if (UD1EquipManagerComponent* EquipManager = GetLyraCharacterFromActorInfo()->GetComponentByClass<UD1EquipManagerComponent>())
	{
...
void UD1GameplayAbility_ThrowTorch::OnNetSync()
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, false);
}

FTransform UD1GameplayAbility_ThrowTorch::GetSpawnTransform()
{
...
void UD1GameplayAbility_ThrowTorch::SpawnThrowableItem()
{
