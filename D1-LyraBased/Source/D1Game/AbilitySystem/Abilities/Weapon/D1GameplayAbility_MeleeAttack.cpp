...
#include UE_INLINE_GENERATED_CPP_BY_NAME(D1GameplayAbility_MeleeAttack)

UD1GameplayAbility_MeleeAttack::UD1GameplayAbility_MeleeAttack(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	ActivationGroup = ELyraAbilityActivationGroup::Exclusive_Replaceable;
}

void UD1GameplayAbility_MeleeAttack::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UD1GameplayAbility_MeleeAttack::OnTargetDataReady(FGameplayEventData Payload)
{
	if (bBlocked)
		return;
...
void UD1GameplayAbility_MeleeAttack::OnMontageEventTriggered(FGameplayEventData Payload)
{
	HandleMontageEvent(Payload);
}

void UD1GameplayAbility_MeleeAttack::HandleMontageEvent(FGameplayEventData Payload)
{
	OnMontageFinished();
}

void UD1GameplayAbility_MeleeAttack::OnMontageFinished()
{
