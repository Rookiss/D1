...
#include UE_INLINE_GENERATED_CPP_BY_NAME(D1GameplayAbility_Melee)

UD1GameplayAbility_Melee::UD1GameplayAbility_Melee(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	
}

void UD1GameplayAbility_Melee::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UD1GameplayAbility_Melee::ParseTargetData(const FGameplayAbilityTargetDataHandle& InTargetDataHandle, TArray<int32>& OutCharacterHitIndexes, TArray<int32>& OutBlockHitIndexes)
{
...
void UD1GameplayAbility_Melee::ProcessHitResult(FHitResult HitResult, float Damage, bool bBlockingHit, UAnimMontage* BackwardMontage, AD1EquipmentBase* WeaponActor)
{
...
void UD1GameplayAbility_Melee::ResetHitActors()
{
	CachedHitActors.Reset();
}

void UD1GameplayAbility_Melee::DrawDebugHitPoint(const FHitResult& HitResult)
{
#if UE_EDITOR
	if (GIsEditor)
	{
...
bool UD1GameplayAbility_Melee::IsCharacterBlockingHit(ALyraCharacter* TargetCharacter)
{
