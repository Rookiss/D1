#include "D1GameplayAbility_MeleeCombo.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(D1GameplayAbility_MeleeCombo)

UD1GameplayAbility_MeleeCombo::UD1GameplayAbility_MeleeCombo(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	ActivationGroup = ELyraAbilityActivationGroup::Exclusive_Replaceable;
}

void UD1GameplayAbility_MeleeCombo::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UD1GameplayAbility_MeleeCombo::HandleMontageEvent(FGameplayEventData Payload)
{
...
void UD1GameplayAbility_MeleeCombo::WaitInputContinue()
{
...
void UD1GameplayAbility_MeleeCombo::WaitInputStop()
{
...
void UD1GameplayAbility_MeleeCombo::OnInputReleased(float TimeHeld)
{
	bInputReleased = true;
}

void UD1GameplayAbility_MeleeCombo::OnInputStart()
{
	bInputPressed = true;
	WaitInputContinue();
}

void UD1GameplayAbility_MeleeCombo::OnInputCancel()
{
	bInputPressed = false;
	WaitInputStop();
}
