...
#include UE_INLINE_GENERATED_CPP_BY_NAME(D1SkillSelectWidget)

UD1SkillSelectWidget::UD1SkillSelectWidget(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	
}

void UD1SkillSelectWidget::NativeConstruct()
{
	...
	Text_SkillName->SetText(FText::GetEmpty());
	
	OnVisibilityChanged.AddUniqueDynamic(this, &ThisClass::HandleVisibilityChanged);
}

FReply UD1SkillSelectWidget::NativeOnMouseMove(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
	...
	if (LengthSquared >= DeadZoneRadiusSquared)
	{
...
void UD1SkillSelectWidget::HandleVisibilityChanged(ESlateVisibility InVisibility)
{
...
void UD1SkillSelectWidget::InitializeUI()
{
...
void UD1SkillSelectWidget::RefreshUI()
{
	if (SelectedIndex == -1)
	{
		Text_SkillName->SetText(FText::GetEmpty());
	...
	{
		Text_SkillName->SetText(AbilityUIInfos[SelectedIndex].AbilityName);
	}
}
